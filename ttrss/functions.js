...
function gotoMain() {
	document.location.href = "feedinator.php";
}
...
function getActiveCatId() {
	return getCookie("ttrss_vf_actcat");
}
function setActiveFeedId(id) {
	setCookie("ttrss_vf_actcat","");
	return setCookie("ttrss_vf_actfeed", id);
}
function setActiveCatId(id) {
	setCookie("ttrss_vf_actfeed","");
	return setCookie("ttrss_vf_actcat",id);
}
...
function showExtendedContentPane(feed_id) {
        ...
        document.getElementById("extendedContentPane").innerHTML = "Loading page content, please wait...";
        xmlhttp_rpc.open("GET", "backend.php?op=show_extended&feed_id="+feed_id,true);
        xmlhttp_rpc.onreadystatechange=extendedpane_callback;
        xmlhttp_rpc.send(null);

}
function extendedpane_callback() {
        var container = document.getElementById('extendedContentPane');
        if (xmlhttp_rpc.readyState == 4) {
                container.innerHTML=xmlhttp_rpc.responseText;
	}

}
function feedframe_callback() {
  var container=document.getElementById('feeds-div');
  if (xmlhttp_rpc.readyState == 4) {
    container.innerHTML=xmlhttp_rpc.responseText;
	}
}
