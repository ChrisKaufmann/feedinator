...
function feedList()
{
	document.getElementById('feeds_status').innerHTML='<img src="/images/mozilla_giallo.gif" height="10">';
	current_view='feed';
	var url='op=feed_list';
	$.ajax({type: "GET",url: backend, data:url,success:function(html){$('#feeds_div').html(html);$('#feeds_status').html='';}})
}
...
function categoryList(id)
{
	...
	$.ajax({type: "GET",url: backend, data:url,success:function(html){$('#feeds_div').html(html);	document.getElementById('feeds_status').innerHTML='';
}})
}
...
function marked_entries()
{
	...
	$.ajax({
		...
		url: backend,
		data: url,
		success: function(html){$('#entries_list_div').html(html);}
	});
